...
/**
 * Override the calendar view page
 *
 * @package     theme_envf
...
if (isloggedin() && (!empty($current)) &&
    ...
    foreach ($current as $c) {
        ...
        $choicetexts[] = $OUTPUT->pix_icon('t/check', '')
            . format_string(choice_get_option_text($choice, $c->optionid));
    }
    echo $OUTPUT->box(
        html_writer::alist($choicetexts, array('class' => 'list-unstyled')), 'generalbox', 'yourselection');
    // END ENVF Modifications
...
// ENVF Modifications
if (false && choice_can_view_results($choice, $current, $choiceopen)) {
